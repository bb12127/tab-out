/* ================================================================
   tabs.js — chrome.tabs / chrome.windows wrappers

   Wraps every direct Chrome tab API call. The current `openTabs`
   snapshot is private to this module; callers read it via
   getOpenTabs(). Every mutation function refreshes the snapshot
   so callers always see post-mutation state.
   ================================================================ */

'use strict';


// All open tabs — populated by fetchOpenTabs(). Private to this module.
let openTabs = [];

/**
 * getOpenTabs() — current snapshot of all browser tabs.
 *
 * Returns the same reference each time fetchOpenTabs() runs, so
 * callers should NOT mutate it. Read-only.
 */
 ...
 * pages, about:blank, etc. Operates on the cached snapshot from
 * the last fetchOpenTabs() call.
 */
